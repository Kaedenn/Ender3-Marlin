...
bool MarlinUI::detected() { return true; }
  ...
  void MarlinUI::draw_marlin_bootscreen(const bool line2/*=false*/) {
      ...
      #define BOOTSCREEN_LINE2 STRING_SPLASH_LINE3
    #else
      #define BOOTSCREEN_LINE2 MARLIN_WEBSITE_URL
    #endif

    // Determine text space needed
    constexpr u8g_uint_t text_width_1 = u8g_uint_t((sizeof(SHORT_BUILD_VERSION) - 1) * (MENU_FONT_WIDTH)),
                         text_width_2 = u8g_uint_t((sizeof(BOOTSCREEN_LINE2) - 1) * (MENU_FONT_WIDTH)),
    ...
    auto _draw_bootscreen_bmp = [&](const uint8_t *bitmap) {
      ...
      if (!two_part || line2) lcd_put_u8str(txt_offx_2, txt_base, F(BOOTSCREEN_LINE2));
    };

    #undef BOOTSCREEN_LINE2
